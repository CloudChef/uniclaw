...
from app.uniclaw.auth.models import UserInfo, ANONYMOUS_USER, AuthenticationError
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Anonymous mode: no auth configured in uniclaw.json
...
def setup_auth_middleware(
    ...
    Args:
        ...
        auth_config: AuthConfig loaded from uniclaw.json, or None for anonymous.
        shadow_store: Optional ShadowUserStore override (for testing).
    """
    from app.uniclaw.auth.strategy import create_auth_strategy
    from app.uniclaw.auth.config import AuthConfig

    if auth_config is None:
        # No auth section in uniclaw.json → anonymous fallback
    ...
    # Coerce raw dict from uniclaw.json into AuthConfig if needed
    ...
    if strategy is None:
        ...
        app.add_middleware(
            ...
            strategy=AuthStrategy(
                provider=__import__(
                    "app.uniclaw.auth.providers.none", fromlist=["NoneProvider"]
                ).NoneProvider(),
                shadow_store=__import__(
                    "app.uniclaw.auth.shadow_store", fromlist=["ShadowUserStore"]
                ).ShadowUserStore(),
            ),
