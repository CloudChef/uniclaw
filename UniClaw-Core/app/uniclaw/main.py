# -*- coding: utf-8 -*-
"""
FastAPI application entry point for UniClaw.
...
Usage:
    uvicorn app.uniclaw.main:app --host 0.0.0.0 --port 8000
"""
...
from app.uniclaw.tools.registration import register_builtin_tools
...
async def lifespan(app: FastAPI):
    ...
    print(f"[UniClaw] Registered {len(registered_tools)} built-in tools")
    ...
    user_skills = Path.home() / ".uniclaw" / "skills"
    ...
    if not provider_config:
        raise RuntimeError(
            f"Provider '{provider}' not configured in uniclaw.json. "
    ...
    if not base_url:
        raise RuntimeError(
            ...
            f"Set environment variable or configure in uniclaw.json"
        )
    if not api_key:
        raise RuntimeError(
    ...
    print("[UniClaw] Application started successfully")
    print(f"[UniClaw] Session storage: {_session_manager.sessions_dir}")
    print(f"[UniClaw] Skills loaded: {len(_skill_registry.list_skills())} executable, {len(_skill_registry.list_md_skills())} markdown")
    
    yield
    
    # Cleanup on shutdown
    print("[UniClaw] Application shutting down")


def create_app() -> FastAPI:
    """Create and configure the FastAPI application."""
    app = FastAPI(
        title="UniClaw Enterprise Assistant",
