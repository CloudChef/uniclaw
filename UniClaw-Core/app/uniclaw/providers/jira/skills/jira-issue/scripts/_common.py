...
def project_root() -> Path:
    ...
    for p in [here] + list(here.parents):
        if (p / "uniclaw.json").exists():
            return p
    raise RuntimeError("uniclaw.json not found from script path")


def load_jira_instance() -> dict[str, Any]:
    cfg_path = project_root() / "uniclaw.json"
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    jira = cfg.get("service_providers", {}).get("jira", {})
    if not isinstance(jira, dict) or not jira:
        raise RuntimeError("service_providers.jira not configured in uniclaw.json")
