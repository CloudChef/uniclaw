...
from app.uniclaw.auth.models import AuthResult, UserInfo, ANONYMOUS_USER, AuthenticationError
from app.uniclaw.auth.middleware import setup_auth_middleware
from app.uniclaw.auth.config import AuthConfig, SmartCMPAuthConfig
from app.uniclaw.auth.strategy import AuthStrategy
from app.uniclaw.auth.shadow_store import ShadowUserStore
...
class TestAuthMiddleware:
    ...
    def test_no_auth_config_injects_anonymous(self):
        ...
        app = FastAPI()
        setup_auth_middleware(app, auth_config=None)
    ...
    def test_provider_none_uses_default_user_id(self, tmp_path):
        ...
        cfg = AuthConfig(provider="none")
        store = ShadowUserStore(store_path=str(tmp_path / "users.json"))
        from app.uniclaw.auth.providers.none import NoneProvider
        from app.uniclaw.auth.strategy import AuthStrategy
        ...
        app = FastAPI()
        setup_auth_middleware(app, auth_config=cfg, strategy=strategy)
    ...
    def test_missing_token_returns_401(self, tmp_path):
        ...
        cfg = AuthConfig(provider="smartcmp")
        store = ShadowUserStore(store_path=str(tmp_path / "users.json"))

        from app.uniclaw.auth.providers.smartcmp import SmartCMPProvider
        from app.uniclaw.auth.config import SmartCMPAuthConfig
        prov = SmartCMPProvider(SmartCMPAuthConfig(validate_url="http://example.com/v"))
    ...
    def test_invalid_token_returns_401(self, tmp_path):
        ...
        from fastapi import FastAPI, Request
        from fastapi.testclient import TestClient
        from app.uniclaw.auth.providers.base import AuthProvider
