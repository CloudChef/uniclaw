...
Run:
    python -m pytest tests/uniclaw/providers/test_jira_e2e.py -v -s

Config is read from UNICLAW_CONFIG (default: tests/uniclaw.test.json) -> service_providers.jira -> first instance.
...
def jira_config() -> dict:
    """Load JIRA connection config from UNICLAW_CONFIG (default tests/uniclaw.test.json)."""
    config_path = Path(
        os.environ.get("UNICLAW_CONFIG", str(_ROOT / "tests" / "uniclaw.test.json"))
    ).resolve()
...
def api_base_url() -> str:
    ...
    return os.environ.get("UNICLAW_API_URL", "http://127.0.0.1:8000")
...
class TestJiraAgentE2E:
    ...
    Prerequisites:
    1. Service must be running with uniclaw.test.json config:
       $env:UNICLAW_CONFIG="uniclaw.test.json"
       uvicorn app.uniclaw.main:app --host 0.0.0.0 --port 8000
    
    2. Jira provider must be configured in uniclaw.test.json
    """
