...
from app.uniclaw.skills.frontmatter import FrontmatterResult, parse_frontmatter
...
from app.uniclaw.agent.prompt_builder import PromptBuilder, PromptBuilderConfig, PromptMode
from app.uniclaw.core.config_schema import SkillsConfig, UniclawConfig
...
class TestPromptBuilderMdSkills:
    ...
    def test_path_compression(self):
        """路径压缩：用户主目录替换为 ~"""
        home = str(Path.home())
        skill = _make_md_skill(file_path=f"{home}/.uniclaw/skills/jira/SKILL.md")
        b = self._builder()
        output = b._build_md_skills_index([skill])

        assert "~/.uniclaw/skills/jira/SKILL.md" in output
...
class TestConfigSchema:
    ...
    def test_uniclaw_config_integration(self):
        """UniclawConfig 集成验证"""
        ucfg = UniclawConfig()
