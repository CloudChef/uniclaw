...
from app.uniclaw.api.routes import APIContext, create_router, set_api_context
from app.uniclaw.api.webhook_dispatch import WebhookDispatchManager
from app.uniclaw.core.config_schema import (
    WebhookConfig,
...
def _build_client(tmp_path: Path, monkeypatch, *, allowed_skills: list[str]) -> tuple[TestClient, _RecordingAgentRunner]:
    monkeypatch.setenv("UNICLAW_WEBHOOK_SK_SMARTCMP_PREAPPROVAL", "secret-1")
    ...
    webhook_config = WebhookConfig(
        ...
        systems=[
            WebhookSystemConfig(
                ...
                enabled=True,
                sk_env="UNICLAW_WEBHOOK_SK_SMARTCMP_PREAPPROVAL",
...
class TestWebhookDispatchManager:
    def test_validate_startup_requires_env_secret(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("UNICLAW_WEBHOOK_SK_SMARTCMP_PREAPPROVAL", raising=False)
        ...
        try:
            ...
            assert "UNICLAW_WEBHOOK_SK_SMARTCMP_PREAPPROVAL" in str(exc)
        else:
