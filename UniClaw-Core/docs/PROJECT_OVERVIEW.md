...
### 1. Agent Runner (`app/uniclaw/agent/runner.py`)
...
### 2. Prompt Builder (`app/uniclaw/agent/prompt_builder.py`)
...
### 3. Skill Registry (`app/uniclaw/skills/registry.py`)
...
### 4. Session Manager (`app/uniclaw/session/manager.py`)
...
### 5. Memory System (`app/uniclaw/memory/manager.py`)
...
2. **Environment variables** (`UNICLAW_*` prefix)
3. **Config file** (`uniclaw.json` / `uniclaw.yaml`)
...
Format: `UNICLAW_<PATH>__<KEY>`

```bash
UNICLAW_AGENT_DEFAULTS__TIMEOUT_SECONDS=300
UNICLAW_MODEL__TEMPERATURE=0.5
```
...
# Run with coverage
pytest --cov=app.uniclaw --cov-report=term-missing
```
